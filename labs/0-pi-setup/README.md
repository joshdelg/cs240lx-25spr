...
Checkoff:
  - Running `make` in `0-hello` should compile and run a hello-world
    program on your pi.
  - Running `make` in `1-hello` should compile and run a hello-world
    program on your pi.
...
3.  Copy all the files from the `firmware` directory at the top level of 
    the class repo to the SD card. (I.e., at `cs240lx-25win/firmware`.)
...
(in `cs240lx-25win/bin`) to run the precompiled hello world program
...
------------------------------------------------------------------------
## 5. Set your `CS240LX_2025_PATH`

Checkoff:
   - Typing:

            cd $CS240LX_2025_PATH/labs/0-pi-setup
...
`CS240LX_2025_PATH` to contain where you have your class repository.
...
E.g., for me, since I'm running `tcsh` I edit my `.tcshrc` file and add:
    ...
    setenv CS240LX_2025_PATH /home/engler/class/cs240lx-25win/

To the end of it.  If you use `bash` you'll do an:

    export CS240LX_2025_PATH=<path to your repo>
...
If this doesn't work:
  ...
  2. You mispelled `CS240LX_2025_PATH` or gave it the wrong path.
