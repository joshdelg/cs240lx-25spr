...
// @joshdelg New for 12-memcheck-trap-II
...
unsigned ck_ptr_in_block_hdr_redzone(hdr_t *h, void *ptr) {
    return (void *) h <= ptr && ptr < ck_data_end(h) + REDZONE_NBYTES;
}
...
hdr_t *ck_get_containing_blk(void *addr) {
    // Walk through allocated list
    for(hdr_t *h = ck_first_alloc(); h; h = ck_next_hdr(h)) {
        if(ck_ptr_in_block_hdr_redzone(h, addr)) {
            return h;
        }
    }

    // Walk through free list
    for(hdr_t *h = free_list; h; h = ck_next_hdr(h)) {
        if(ck_ptr_in_block_hdr_redzone(h, addr)) {
            return h;
        }
    }
