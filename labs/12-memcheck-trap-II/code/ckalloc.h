...
static inline int ck_illegal_offset(hdr_t *h, void *addr) {
    char *s = ck_data_start(h); // start of allocated memory
    char *e = ck_data_end(h); // end of allocated memory
    char *p = addr;
...
// get the containing block --- includes redzones.
hdr_t *ck_get_containing_blk(void *addr);

// allocator fn type
typedef void *(*alloc_t)(unsigned);
// free fn type
typedef void (*free_t)(void *);

// call to set <alloc> and <free>
void ck_init(alloc_t alloc_fn, free_t free_fn);
