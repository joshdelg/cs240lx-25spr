...
void notmain(void) {
    ...
    neo_t h = neopix_init(21, 30);
    neopix_fast_clear(h, 30);

    uint32_t max_turns = 15; // Use 2 lights per new one
    uint32_t light_history[max_turns];

    // Flash new round
    for(int t = 0; t < 2; t++) {
        for(int i = 0; i < 30; i++) {
            for(int j = 0; j <= i; j++) {
                neopix_write(h, j, 255, 255, 255);
            }
            neopix_flush(h);
            delay_ms(50);
        }
    ...
    for(int turn = 0; turn < max_turns; turn++) {
        // Get new light
        uint32_t new_light = rpi_rand16() % 4;
        light_history[turn] = new_light;

        // Flash sequence
        output("Flashing sequence of length %d\n", turn + 1);
        for(int i = 0; i <= turn; i++) {
            for(int j = 0; j <= i; j++) {
                neopix_write(h, j, light_colors[light_history[j]][0], light_colors[light_history[j]][1], light_colors[light_history[j]][2]);
            }
            neopix_flush(h);
            delay_ms(500);
        }
        ...
        // Get sequence input from player
        for(int i = 0; i <= turn; i++) {
            uint32_t key = capture_key_press();
            while(key != UP && key != DOWN && key != LEFT && key != RIGHT) key = capture_key_press();
            
            log_key_press(key);

            if(key != color_key[light_history[i]]) {
                output("Wrong sequence! Game over.\n");

                // Flash wrong light
                for(int k = 0; k < 30; k++) {
                    for(int kk = 0; kk <= k; kk++) {
                        neopix_write(h, kk, 255, 0, 0);
                    }
                    neopix_flush(h);
                    delay_ms(50);
                }
            ...
            // Flash correct light (add it to the sequence))
        ...
        // After inputting correct sequence flash new round
        for(int i = 0; i < 30; i++) {
            for(int j = 0; j <= i; j++) {
                neopix_write(h, j, 255, 255, 255);
            }
