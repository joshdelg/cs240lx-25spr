...
void _cstart() {
    ...
    // while( bss < bss_end )
    //     *bss++ = 0; --> will be done in the ELF loader
    gcc_mb();
    ...
    // Stuart: nuked this for the final project libpi.so (will be done in main executable's _start)
    // uart_init(); 
    ...
    // Stuart: nuked this for the final project libpi.so (no need to check for redzone)
    // redzone_init(); 
