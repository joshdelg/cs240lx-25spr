...
// code at.
void simple_boot(int fd, uint32_t boot_addr, const uint8_t *buf, unsigned n, const uint8_t *debug_buf, unsigned debug_n) { 
    ...
    unsigned code_crc32 = our_crc32(buf, n);
    ...
    boot_check(fd, "crc32 did not match :(", code_crc32, trace_get32(fd));
    ...
    // @joshdelg Now, send the code for the debug target
    while((op = get_op(fd)) != GET_DEBUG_INFO) {}

    trace_put32(fd, PUT_DEBUG_INFO);
    trace_put32(fd, DEBUGBASE);
    trace_put32(fd, debug_n);
    trace_put32(fd, our_crc32(debug_buf, debug_n));

    while(get_op(fd) != GET_DEBUG_CODE) {}

    boot_check(fd, "crc32 did not match :(", our_crc32(debug_buf, debug_n), trace_get32(fd));

    trace_put32(fd, PUT_DEBUG_CODE);

    for(int i = 0; i < debug_n; i++) {
        trace_put8(fd, debug_buf[i]);
    }
